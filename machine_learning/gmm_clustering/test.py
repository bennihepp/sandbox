...
def create_random_dataset(n=1000, k=5, sigma=1.0, prior_mu=0.0, prior_sigma=10.0, pi=None):
    ...
    for i,p in enumerate( pi ):
        print i,p
        if p > 0:
            X[ j:j+p, 0 ] = np.random.normal( mu[ i ], sigma, p )
            c[ j:j+p ] = i
        j += p
    """for i in xrange( k ):
        c_mask = ( c == i )
...
colors = 'rgbmc'
for i,(m,s,p) in enumerate(zip(mu,sigma,pi)):
    print 'i:', i, m, s, p
    m = m[0]
    s = s[0,0]
    A = np.linspace( m - 10*s, m + 10*s, 100 )
    B = 1/np.sqrt(2*np.pi*s**2)*np.exp(-(A-m)**2/(2*s**2))
    B *= p * N
    c = colors[ i % len( colors ) ]
    plt.plot( A, B, '%s-' % c )
plt.hist( X, bins=50, alpha=0.5, facecolor='lightgrey' )
